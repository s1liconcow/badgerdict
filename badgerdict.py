...
class BadgerDict:
    ...
    def __init__(self, path: Optional[str] = None, *, in_memory: bool = False, lib_path: Optional[str] = None) -> None:
    ...
    def _coerce_bytes(value: BytesLike) -> bytes:
        ...
        raise TypeError(f"Unsupported value type {type(value)!r}. Expected bytes or str.")
    ...
    def __getitem__(self, key: BytesLike) -> bytes:
        return self.get(key, raise_missing=True)

    def __setitem__(self, key: BytesLike, value: BytesLike) -> None:
        self.set(key, value)

    def __delitem__(self, key: BytesLike) -> None:
        if not self.delete(key):
            raise KeyError(key)

    def __contains__(self, key: BytesLike) -> bool:
    ...
    def _encode_key(self, key: BytesLike) -> bytes:
        data = self._coerce_bytes(key)
        if not data:
            raise ValueError("empty keys are not supported")
        return data

    def set(self, key: BytesLike, value: BytesLike) -> None:
        key_bytes = self._encode_key(key)
        value_bytes = self._coerce_bytes(value)
        status = self._call(
            "Set",
    ...
    def get(self, key: BytesLike, default: Optional[bytes] = None, *, raise_missing: bool = False) -> Optional[bytes]:
        key_bytes = self._encode_key(key)
        ...
        try:
            result = ctypes.string_at(ptr, value_len.value)
        finally:
            self._lib.FreeBuffer(ptr)
        return result

    def delete(self, key: BytesLike) -> bool:
        key_bytes = self._encode_key(key)
        status = self._call(
            "Delete",
            ctypes.c_size_t(self._handle),
